...
```json
{
  "completion_action": {
    ...
    "open_in_new_tab": true,
    "redirect_params": [
      { "key": "email", "source_field": "correo_contacto" },
      { "key": "empresa", "source_field": "nombre_empresa" }
    ],
...
- `redirect_params`: optional array of query-param mappings for redirect mode.
- `redirect_params[].key`: query parameter name to append in final redirect URL.
- `redirect_params[].source_field`: question `id` whose submitted answer will be used as value.
